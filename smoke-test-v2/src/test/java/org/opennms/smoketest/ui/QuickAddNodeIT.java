...
public class QuickAddNodeIT extends OpenNMSSeleniumIT {
    ...
    protected WebElement enterTextAutocomplete(final By selector, final CharSequence... text) throws InterruptedException {
        final WebElement element = OpenNMSSeleniumIT.driver.findElement(selector);
        element.clear();
        element.click();
        Thread.sleep(500);
        ...
        try {
            ...
            final List<WebElement> matching = OpenNMSSeleniumIT.driver.findElements(By.cssSelector("a[title='"+text+"']"));
            if (!matching.isEmpty()) {
