...
public class AngularLoginRedirectIT extends OpenNMSSeleniumIT {
    ...
    private List<Check> checks = Lists.newArrayList(
            ...
            new Check(
                    ...
                    () -> {
                        ...
                        sleep(SLEEP_TIME); // encounter for UI Delay
                        OpenNMSSeleniumIT.driver.findElement(By.xpath("//button[@data-bb-handler='reloadAll']")).click();
                        sleep(SLEEP_TIME); // encounter for UI Delay
                        OpenNMSSeleniumIT.driver.findElement(By.xpath("//button[@data-bb-handler='confirm']")).click();
                        sleep(SLEEP_TIME); // encounter for UI Delay
                    }),
            new Check(
    ...
    public void verifyRedirectToLogin() throws IOException {
        for (Check eachCheck : checks) {
            ...
            // Go to Page
            login();
            OpenNMSSeleniumIT.driver.get(getBaseUrlInternal() + "opennms/" + eachCheck.url);
            ...
            new WebDriverWait(OpenNMSSeleniumIT.driver, 5).until(input -> {
                    LOG.info("{}: Verify redirect to login.jsp occurred", eachCheck.url);
                    return Objects.equals(getBaseUrlInternal() + "opennms/login.jsp?session_expired=true", OpenNMSSeleniumIT.driver.getCurrentUrl());
                }
            );
    ...
    private void simulateSessionTimeout() throws IOException {
        final Set<Cookie> cookies = OpenNMSSeleniumIT.driver.manage().getCookies();
        ...
        OpenNMSSeleniumIT.driver.manage().deleteAllCookies();
    }
}
