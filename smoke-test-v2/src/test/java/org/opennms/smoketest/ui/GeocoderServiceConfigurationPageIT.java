...
public class GeocoderServiceConfigurationPageIT extends UiPageTest {
    ...
    private void resetConfiguration() {
        environment.opennms().getRestClient().resetGeocoderConfiguration();
    }
    ...
    private class Page {
        ...
        public Page open() {
            ...
            new WebDriverWait(OpenNMSSeleniumIT.driver, 5).until(driver -> getTabs().size() == expectedTabs.size());
            return this;
        }
    ...
    private class Tab {
        ...
        public WebElement getElement() {
            return execute(() -> OpenNMSSeleniumIT.driver.findElement(By.xpath(String.format(TAB_XPATH, name))));
        }
    ...
    private class SettingsTab extends Tab {
        ...
        public String getActiveGeocoder() {
            return execute(() -> {
                final List<WebElement> elements = OpenNMSSeleniumIT.driver.findElements(By.xpath("//div[contains(@class, 'toggle') and not(contains(@class,'off')) and not (contains(@class, 'toggle-group'))]/./.."));
                if (elements.isEmpty()) {
                    return null;
                }
    ...
    private abstract class GeocoderTab extends Tab {
        ...
        public boolean isConfiguredProperly() {
            return execute(() -> OpenNMSSeleniumIT.driver.findElements(By.xpath(String.format(TAB_XPATH, getName()) + "//i[@class='fa fa-exclamation-triangle']")).isEmpty());
        }

        public boolean isDirty() {
            boolean hasChanges = execute(() -> OpenNMSSeleniumIT.driver.findElements(By.xpath("//p[text() = 'You have unsaved changes' and contains(@class, 'text-warning')]"))).isEmpty() == false;
            return hasChanges;
        }

        public void update() {
           getSaveButtonElement().click();
           new WebDriverWait(OpenNMSSeleniumIT.driver, 5, 500).until(webDriver -> !isDirty());
        }

        WebElement getSaveButtonElement() {
            return execute(() -> OpenNMSSeleniumIT.driver.findElement(By.id("saveButton")));
        }
    }
    ...
    private class GoogleTab extends GeocoderTab {
        ...
        public GoogleAuthMode getAuthMode() {
            if (execute(() -> OpenNMSSeleniumIT.driver.findElement(By.id("googleClientIdAuthentication"))).isSelected()) {
        ...
        public void setAuthMode(GoogleAuthMode authMode) {
            if (authMode != getAuthMode()) {
                execute(() -> OpenNMSSeleniumIT.driver.findElement(By.id("googleClientIdAuthentication"))).click();
            }
        }
    }
}
