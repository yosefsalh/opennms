...
public class ChooseResourcesPageIT extends OpenNMSSeleniumIT {
    ...
    public void verifyGraphAll() throws Exception {
        try {
            ...
            OpenNMSSeleniumIT.driver.get(getBaseUrlInternal() + "opennms/element/node.jsp?node=SeleniumTestGroup:localhost");
            ...
            Assert.assertTrue("There should be graphs visible, but could not find any", !OpenNMSSeleniumIT.driver.findElements(By.xpath("//div[@id='graph-results']//div[@class='graph-container']")).isEmpty());
        } finally {
            // CLEANUP
    ...
    public void verifyShowsNoResourcesBanner() throws Exception {
        try {
            ...
            OpenNMSSeleniumIT.driver.get(getBaseUrlInternal() + "opennms/element/node.jsp?node=SmokeTests:TestMachine1");
