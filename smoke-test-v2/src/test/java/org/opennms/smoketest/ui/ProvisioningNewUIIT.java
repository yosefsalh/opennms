...
//        deleteTestRequisition();
    ...
    public void testRequisitionUI() throws Exception {
        ...
        OpenNMSSeleniumIT.driver.get(getBaseUrlInternal() + "opennms/rest/nodes/" + REQUISITION_NAME + ":" + NODE_FOREIGNID + "/ipinterfaces/" + NODE_IPADDR + "/services/ICMP");
        OpenNMSSeleniumIT.driver.manage().timeouts().implicitlyWait(2000, TimeUnit.MILLISECONDS);
        try {
            for (int i=0; i<30; i++) {
                try {
                    final WebElement e = OpenNMSSeleniumIT.driver.findElement(By.xpath("//service/serviceType/name[text()='ICMP']"));
                    if (e != null) {
                        break;
                    }
                } catch (Exception e) {}
                OpenNMSSeleniumIT.driver.navigate().refresh();
            }
        } finally {
            OpenNMSSeleniumIT.driver.manage().timeouts().implicitlyWait(LOAD_TIMEOUT, TimeUnit.MILLISECONDS);
        }

        // Open the nodes list page
        OpenNMSSeleniumIT.driver.get(getBaseUrlInternal() + "opennms/");
        clickMenuItem("Info", "Nodes", "element/nodeList.htm");

        try {
            // Disable implicitlyWait
            OpenNMSSeleniumIT.driver.manage().timeouts().implicitlyWait(0, TimeUnit.MILLISECONDS);
        ...
        OpenNMSSeleniumIT.driver.get(getBaseUrlInternal() + "opennms/element/node.jsp?node=" + REQUISITION_NAME + ":" + NODE_FOREIGNID);
