...
public class GetDeviceConfig implements Action {
    ...
    public Object execute() throws Exception {
        ...
        while (true) {
            ...
            CompletableFuture<byte[]> future = getDeviceConfig(host, location, configType, timeout);
            try {
                try {
    ...
    CompletableFuture<byte[]> getDeviceConfig(String ipAddress, String location, String configType, int timeout) {
        CompletableFuture<byte[]> future = new CompletableFuture<>();
        try {
            Optional<byte[]> dc = deviceConfigService.getDeviceConfig(ipAddress, location, configType, timeout);
            if (dc.isPresent()) {
                future.complete(dc.get());
            } else {
                future.completeExceptionally(new IOException("couldn't get device config, reason unknown"));
            }
        } catch (IOException e) {
